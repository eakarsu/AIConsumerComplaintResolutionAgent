/**
 * Custom Feature: complaint-triage
 * Complaint auto-routing & triage with NLP
 *
 * POST /api/ai/complaint-triage
 * Auth required. Generated as part of Custom Feature Suggestions scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_COMPLAINT_TRIAGE_KEY
...
const FEATURE_KEY = process.env.FEATURE_COMPLAINT_TRIAGE_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Complaint auto-routing & triage with NLP.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIConsumerComplaintResolutionAgent - complaint-triage',
    },
    body: JSON.stringify({
...
router.post('/complaint-triage', auth, async (req, res) => {
  try {
    ...
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_COMPLAINT_TRIAGE_KEY');
    const ai = await callLLM({ feature: 'complaint-triage', goal: 'Complaint auto-routing & triage with NLP', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['complaint-triage', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[complaint-triage] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'complaint-triage',
      endpoint: '/api/ai/complaint-triage',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[complaint-triage] error:', err.message);
    return res.status(err.statusCode || 500).json({ error: err.message || 'Internal error' });
  }
});

router.get('/complaint-triage/health', (req, res) => {
  res.json({
    feature: 'complaint-triage',
    endpoint: '/api/ai/complaint-triage',
