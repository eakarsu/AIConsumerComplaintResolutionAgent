 ...
 * Sentiment-driven real-time escalation
 *
...
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Sentiment-driven real-time escalation.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIConsumerComplaintResolutionAgent - sentiment-escalate',
    },
    body: JSON.stringify({
...
router.post('/sentiment-escalate', auth, async (req, res) => {
  try {
    ...
    const ai = await callLLM({ feature: 'sentiment-escalate', goal: 'Sentiment-driven real-time escalation', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['sentiment-escalate', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[sentiment-escalate] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
