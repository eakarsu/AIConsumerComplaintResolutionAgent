 ...
 * Resolution / customer churn prediction
 *
...
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Resolution / customer churn prediction.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIConsumerComplaintResolutionAgent - churn-prediction',
    },
    body: JSON.stringify({
...
router.post('/churn-prediction', auth, async (req, res) => {
  try {
    ...
    const ai = await callLLM({ feature: 'churn-prediction', goal: 'Resolution / customer churn prediction', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['churn-prediction', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[churn-prediction] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
